...
- ✅ **`NewLogicalDB()`**: Backward compatible, defaults to `PartitionCount=1` (legacy mode)
- ✅ **`LogicalDB.Open()`**: Detects v0.3 vs v0.4 and routes accordingly
  - v0.3: Single `dbname.wal` → forces `PartitionCount=1`, uses `openLegacy()`
  - v0.4: Partitioned WALs (`p0.wal`, `p1.wal`, ...) → uses `openPartitioned()`
...
- ✅ Backward compatible: `PartitionCount=1` uses legacy mode

**v0.3 Detection:**

- ✅ **`LogicalDB.Open()`**:
  - Checks for `dbname.wal` (v0.3) vs `p0.wal` (v0.4)
  - If v0.3 layout detected → forces `PartitionCount=1`, uses `openLegacy()`
  - If v0.4 layout → uses `openPartitioned()`

**Backward Compatibility:**

- ✅ **Legacy mode** (`PartitionCount=1`):
  - Uses single WAL (`dbname.wal`)
  - Uses single datafile (`dbname.data`)
  - Uses single index
  - Uses legacy checkpoint manager
  - Uses legacy WAL trimmer
  - Fully compatible with v0.3

**Migration Path:**

- ✅ v0.3 databases continue to work (detected and opened in legacy mode)
- ✅ New databases default to partitioned mode (v0.4)
- ⏳ Migration tool (v0.3 → v0.4) not yet implemented
...
- ✅ **v0.3 databases**: Automatically detected and opened in legacy mode (`PartitionCount=1`)
- ✅ **IPC protocol**: Existing commands unchanged; new `CmdQuery` added
- ✅ **Client code**: No changes required (routing is transparent)

### Forward Compatibility

- ✅ **v0.4 databases**: Use partitioned layout; can be opened with v0.4+
- ⏳ **Migration tool**: v0.3 → v0.4 migration not yet implemented
...
- ⏳ Query fan-out tests
- ⏳ v0.3 → v0.4 compatibility tests

### Benchmarks

- ⏳ Partitioned vs legacy performance
...
4. **Migration Tool**: v0.3 → v0.4 migration not yet implemented
5. **Tests**: Comprehensive test suite not yet implemented
...
3. **Migration Tool**: v0.3 → v0.4 database migration
4. **Performance Tuning**: Optimize query fan-out, recovery parallelism
5. **Documentation**: API docs, query language spec, migration guide
