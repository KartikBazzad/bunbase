...
// Protocol matches bunder-manager/internal/rpc: length-prefixed frames,
...
// Client is a TCP RPC client for the bunder KV proxy.
type Client struct {
	Addr      string
	conn      net.Conn
	mu        sync.Mutex
...
// New creates a new RPC client. addr is TCP address (e.g. "bunder-manager:9091").
...
// ProxyRequest sends a KV proxy request and returns status code, body, and error.
// path is the suffix after /kv/{projectID}, e.g. /kv/{key} or /keys or /health.
func (c *Client) ProxyRequest(method, projectID, path string, body []byte) (status int, respBody []byte, err error) {
	...
	payload, err := json.Marshal(proxyKVRequest{
		Method:    method,
		ProjectID: projectID,
		Path:      path,
		Body:      bodyB64,
	...
	var kvResp proxyKVResponse
	if err := json.Unmarshal(respPayload, &kvResp); err != nil {
		return 0, nil, fmt.Errorf("decode response: %w", err)
	}
	if respStatus == statusError && kvResp.Error != "" {
		return 0, nil, fmt.Errorf("%s", kvResp.Error)
	}
	if kvResp.Body != "" {
		respBody, _ = base64.StdEncoding.DecodeString(kvResp.Body)
	}
	return kvResp.Status, respBody, nil
}
